...
export default function CotizarVidaClient() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const documento = searchParams.get('documento');
  const edad = searchParams.get('edad');

  const [esFumador, setEsFumador] = useState(false);
  const [polizas, setPolizas] = useState<PolizaVida[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const handleCotizarVida = useCallback(async (fumador: boolean) => {
    setLoading(true);
    setError(null);
    setPolizas([]);

    try {
      const res = await fetch(`/api/cotizar-vida?edad=${edad}&fumador=${fumador}`);
      if (!res.ok) {
        throw new Error('Error al obtener cotizaciones de vida.');
      }
  ...
  useEffect(() => {
    if (edad) {
      handleCotizarVida(esFumador);
    }
  }, [edad, esFumador, handleCotizarVida]);

  const handleVerDetalle = (poliza: PolizaVida) => {
    const polizaData = encodeURIComponent(JSON.stringify(poliza));
    router.push(`/cotizacion/vida/detalle?data=${polizaData}`);
  };
      ...
      <div className="container mx-auto px-6">
        <FadeIn>
          <div className="bg-white rounded-xl shadow-lg p-8 mb-8 max-w-4xl mx-auto">
            <h1 className="text-3xl font-bold text-brand-blue text-center mb-6">Planes de Seguro de Vida</h1>
            <p className="text-center text-brand-gray mb-8">Protege el futuro de los que más quieres. Estos son los planes que encontramos para ti.</p>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center bg-gray-100 p-4 rounded-lg">
                <div className="md:col-span-1">
                    <label className="block text-brand-gray text-sm font-bold mb-1">Documento</label>
                    <input type="text" value={documento || ''} className="w-full px-4 py-2 text-brand-gray bg-gray-200 border rounded-lg" disabled />
                </div>
                <div className="md:col-span-1">
                    <label className="block text-brand-gray text-sm font-bold mb-1">Edad</label>
                    <input type="number" value={edad || ''} className="w-full px-4 py-2 text-brand-gray bg-gray-200 border rounded-lg" disabled />
                </div>
                <div className="md:col-span-1 flex items-center justify-center h-full mt-4">
                    <input id="fumador" type="checkbox" checked={esFumador} onChange={(e) => setEsFumador(e.target.checked)} className="h-5 w-5 text-brand-blue focus:ring-brand-blue border-gray-300 rounded" />
                    <label htmlFor="fumador" className="ml-2 block text-brand-gray font-medium">¿Eres fumador?</label>
                </div>
            </div>
          </div>
        </FadeIn>
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200 hover:border-brand-blue transition relative flex flex-col h-full">
                  <div className="flex-grow">
                    ...
                    <div className="text-brand-gray mb-4">
                        <p><strong>Cobertura por Fallecimiento:</strong> ${poliza.coberturaFallecimiento.toLocaleString('es-CO')}</p>
                        <p><strong>Cobertura por Invalidez:</strong> ${poliza.coberturaInvalidez.toLocaleString('es-CO')}</p>
                    </div>
